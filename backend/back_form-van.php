 ...
 */
if (!empty($_GET['id'])) {
    $stmt = $pdo->conn->prepare('SELECT * FROM van v LEFT JOIN van_time vt ON vt.v_id = v.v_id WHERE v.v_id =:id');
    ...
    $vt_drivestart = $result->vt_drivestart;
    $vt_driveend = $result->vt_driveend;
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        mutiselect = $('.select2').multiSelect({
            selectableHeader: "<div class='btn btn-info btn-block'>สถานที่ในระบบ</div>",
            selectionHeader: "<div class='btn btn-info btn-block'>สถานที่ที่ท่านเลือก</div>",
            //selectableFooter: "<div class='btn btn-info btn-block'>Selectable footer</div>",
            //selectionFooter: "<div class='custom-header'>Selection footer</div>"
            afterSelect: function (values) {
                //alert("Select value: " + values);        
                console.log(values);
                arrayPlaceIdSelect.push(values);
                console.log(arrayPlaceIdSelect);
            },
                ...
                if (arrayChair === 0) {
                    ...
                    if (isConfirm) {
                        $.ajax({
                            ...
                            dataType: 'json',
                            data: {'jsonParameter': arrayObjectToJsonString(getFormValue()),
                            },
                            type: 'post',
    ...
    function setLoadVanForm() {
        ...
        $.get('../actionDb/van_place.php?action=getPlacesByVanId', {
            ...
            mutiselect.multiSelect('select', arrayPlace);
